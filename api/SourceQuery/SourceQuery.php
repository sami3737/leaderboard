<?php
	...
	class SourceQuery
	{
        ...
        const SERVER_API_ADDRESS = '127.0.0.1';
		/**
		 * Engines
		 */
		const GOLDSOURCE = 0;
		...
		const SERVERDATA_ANOTHER_RESPONSE_VALUE = 4;
		 ...
		 */
		public function Connect( $Address, $Port, $RconPort, $Timeout = 3, $Engine = self::SOURCE )
		{
			$this->Disconnect( );
		...
		public function Ping( )
		{
			...
			return $Buffer->GetByte( ) === self::A2S_PING;
		}
		...
		public function GetItem($apikey){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':28015/items.json?apikey='.$apikey);
		...
		public function GetStatus(){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':28022/status.json');
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
		...
		public function GetPlayersGlobalStats(){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':8888/getPlayersGlobalStats');
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
		...
		public function GetConnected($apikey){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':28022/players.json?apikey='.$apikey);
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
		...
		public function SetRconPassword( $Password )
		{
			...
			switch( $this->Socket->Engine )
			{
				...
				{
					$this->Rcon = new SourceRcon( $this->Address, $this->RconPort, $this->Timeout );
